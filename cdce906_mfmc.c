...
mmc_result_t cdce906_read_cfg(uint8_t bus_id, uint8_t chip, cdce906_cfg* cfg)
{
    uint8_t data[27];
    mmc_result_t ret = 0;

    ret = i2c_read(bus_id, chip, 0x00, data, sizeof(data));
...
mmc_result_t cdce906_write_cfg(uint8_t bus_id, uint8_t chip, const cdce906_cfg* cfg)
{
     ...
     */
    tmp[0] = data[CDCE906_PLL2_VCO_MUX_BYTE + 2] | CDCE906_PLL2_VCO_MUX_MASK;
    ret = i2c_write(bus_id, chip, 0x80 | CDCE906_PLL2_VCO_MUX_BYTE, tmp, 1);
     ...
     */
    tmp[0] = data[CDCE906_SSC_FREQ_SEL_BYTE + 2];
    ret = i2c_write(bus_id, chip, 0x80 | CDCE906_SSC_FREQ_SEL_BYTE, tmp, 1);
     ...
     */
    ret = i2c_write(bus_id, chip, data[0], &data[1], sizeof(data) - 2);
